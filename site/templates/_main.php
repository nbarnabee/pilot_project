...
<head>
    ...
    <?php endif; ?>
    <script>
        window.WebsiteConfiguration = {};
    </script>
</head>
<body>
<div id="header">
    <?= $files->render('partials/header.php') ?>
</div>
<div id="content">
    <h1><?= $page->title ?></h1>
</div>
<div id="footer">
    <?= $files->render('partials/footer.php') ?>
</div>
<?php if ($config->reCaptchaEnabled): ?>
    <script src="<?= "https://www.google.com/recaptcha/api.js?render=$config->reCaptchaPublicKey" ?>"></script>
    <script>
        window.WebsiteConfiguration.reCaptchaPublicKey = "<?= $config->reCaptchaPublicKey ?>";
    </script>
<?php endif; ?>
